...
/**
 * Observer que escucha eventos de creación de notificaciones con canal "email" y envía emails.
 * Implementa el patrón Observer para desacoplar la lógica de persistencia de la lógica de envío de emails.
 */
@ApplicationScoped
public class NotificacionEmailObserver extends NotificacionObserverBase {

    private static final Logger LOG = Logger.getLogger(NotificacionEmailObserver.class);
    ...
    public void onNotificacionCreada(@Observes NotificacionCreadaEvent event) {
        ...
        // Solo procesar notificaciones con canal "email"
        if (!"email".equalsIgnoreCase(notificacion.canal)) {
            LOG.debugf("Notificación %d con canal '%s' ignorada por EmailObserver", notificacion.id, notificacion.canal);
            return;
        }
